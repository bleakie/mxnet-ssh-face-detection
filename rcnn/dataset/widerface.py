...
# from numpy import *  # add numpy lib by sai
        ...
        if not self._test_flag:
            while count < len(annos):
                ...
                for i in xrange(n_anno):
                    count += 1
                    annos[count] = annos[count].strip('\n')
                    bbox = annos[count].split(' ')[0:7]#增加blur，invalid，occlusion内容，原来是[0:4]
                    bbox = [int(round(float(x))) for x in bbox]
                    x1 = max(0, bbox[0])
                    y1 = max(0, bbox[1])
                    self._fp_bbox_map[name].append([x1, y1, x1 + bbox[2], y1 + bbox[3], bbox[4], bbox[5], bbox[6]])#blur=bbox[4],invalid=bbox[5],occlusion=bbox[6]
                count += 1
            ...
            if self._test_flag:
                ...
                for bbox in self._fp_bbox_map[fp]:
                    ...
                    if self._image_set=='train':
                      # min_box to add mask with sai
                      if (x2 - x1) < config.TRAIN.MIN_BOX_SIZE or y2 - y1 < config.TRAIN.MIN_BOX_SIZE:
                          continue
                      if config.BBOX_MASK_THRESH>0:
                        size_bbox = min(x2 - x1, y2 - y1)
                        # blur=bbox[4],invalid=bbox[5],occlusion=bbox[6]
                        #invalid>0 or blur>1 or occlusion>1属于无用的照片，加mask
                        occlusion_blur = max(bbox[4], bbox[6])
                        occlusion_blur_invalid = 0
                        if occlusion_blur >1 or bbox[5] > 0:
                            occlusion_blur_invalid = 1
                        if size_bbox < config.BBOX_MASK_THRESH or occlusion_blur_invalid > 0:
